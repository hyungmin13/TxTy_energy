...
def PINN_update(model_state, optimiser_fn, equation_fn, dynamic_param, static_params, static_keys, grids, particles, particle_vel, particle_Tx, particle_Ty, particle_bd, model_fn):
    ...
    lossval, grads = value_and_grad(equation_fn, argnums=0)(dynamic_param, all_params, grids, particles, particle_vel, particle_bd, particle_Tx, particle_Ty, model_fn)
...
class PINN(PINNbase):
    def train(self,numb=0,**kwargs):
        ...
        data_Tx.append(train_data['Tx'][perm_p[i*self.c.optimization_init_kwargs["p_batch"]:(i+1)*self.c.optimization_init_kwargs["p_batch"]], 0:1])
        data_Ty.append(train_data['Ty'][perm_p[i*self.c.optimization_init_kwargs["p_batch"]:(i+1)*self.c.optimization_init_kwargs["p_batch"]], 0:1])
    ...
    def report(self, i, report_fn, dynamic_params, all_params, p_batch, v_batch, g_batch, Tx_batch, Ty_batch, b_batch, valid_data, e_batch_key, save_step, model_fns):
        ...
        if save_report:
            ...
            if 'T' in valid_data.keys():
                ...
                Losses = report_fn(dynamic_params, all_params, g_batch, p_batch, v_batch, e_batch_pos, e_batch_vel, e_batch_T, Tx_batch, Ty_batch, b_batch, model_fns)
            else:
                #e_batch_T = random.choice(e_key, valid_data['T'], shape = (self.c.optimization_init_kwargs["e_batch"],))
