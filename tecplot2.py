...
def Derivatives(dynamic_params, dynamic_params2, all_params, all_params2, g_batch, model_fns, model_fns2):
    keys = ['u_ref', 'v_ref', 'w_ref']
    keys2 = ['p_ref', 'T_ref'] 

    all_params["network1"]["layers"] = dynamic_params
    all_params2["network1"]["layers"] = dynamic_params2
    out_xx, out_xxx = equ_func3(all_params2, g_batch, jnp.tile(jnp.array([[0.0, 1.0, 0.0, 0.0]]),(g_batch.shape[0],1)),
                                                    jnp.tile(jnp.array([[0.0, 1.0, 0.0, 0.0]]),(g_batch.shape[0],1)),
                                                    jnp.tile(jnp.array([[0.0, 1.0, 0.0, 0.0]]),(g_batch.shape[0],1)), model_fns2)
    out_xy, out_xxy = equ_func3(all_params2, g_batch, jnp.tile(jnp.array([[0.0, 1.0, 0.0, 0.0]]),(g_batch.shape[0],1)),
                                                    jnp.tile(jnp.array([[0.0, 0.0, 1.0, 0.0]]),(g_batch.shape[0],1)),
                                                    jnp.tile(jnp.array([[0.0, 1.0, 0.0, 0.0]]),(g_batch.shape[0],1)), model_fns2)
    out_xz, out_xxz = equ_func3(all_params2, g_batch, jnp.tile(jnp.array([[0.0, 1.0, 0.0, 0.0]]),(g_batch.shape[0],1)),
                                                    jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(g_batch.shape[0],1)),
                                                    jnp.tile(jnp.array([[0.0, 1.0, 0.0, 0.0]]),(g_batch.shape[0],1)), model_fns2)
    
    _, out_xyy = equ_func3(all_params2, g_batch, jnp.tile(jnp.array([[0.0, 1.0, 0.0, 0.0]]),(g_batch.shape[0],1)),
                                                jnp.tile(jnp.array([[0.0, 0.0, 1.0, 0.0]]),(g_batch.shape[0],1)),
                                                jnp.tile(jnp.array([[0.0, 0.0, 1.0, 0.0]]),(g_batch.shape[0],1)), model_fns2)
    out_yy, out_yyy = equ_func3(all_params2, g_batch, jnp.tile(jnp.array([[0.0, 0.0, 1.0, 0.0]]),(g_batch.shape[0],1)),
                                                    jnp.tile(jnp.array([[0.0, 0.0, 1.0, 0.0]]),(g_batch.shape[0],1)),
                                                    jnp.tile(jnp.array([[0.0, 0.0, 1.0, 0.0]]),(g_batch.shape[0],1)), model_fns2)
    out_yz, out_yyz = equ_func3(all_params2, g_batch, jnp.tile(jnp.array([[0.0, 0.0, 1.0, 0.0]]),(g_batch.shape[0],1)),
                                                    jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(g_batch.shape[0],1)),
                                                    jnp.tile(jnp.array([[0.0, 0.0, 1.0, 0.0]]),(g_batch.shape[0],1)), model_fns2)
    
    _, out_xzz = equ_func3(all_params2, g_batch, jnp.tile(jnp.array([[0.0, 1.0, 0.0, 0.0]]),(g_batch.shape[0],1)),
                                                jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(g_batch.shape[0],1)),
                                                jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(g_batch.shape[0],1)), model_fns2)
    _, out_yzz = equ_func3(all_params2, g_batch, jnp.tile(jnp.array([[0.0, 0.0, 1.0, 0.0]]),(g_batch.shape[0],1)),
                                                jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(g_batch.shape[0],1)),
                                                jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(g_batch.shape[0],1)), model_fns2)
    out_zz, out_zzz = equ_func3(all_params2, g_batch, jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(g_batch.shape[0],1)),
                                                    jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(g_batch.shape[0],1)),
                                                    jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(g_batch.shape[0],1)), model_fns2)
    
    out_x, out_xt = equ_func2(all_params2, g_batch, jnp.tile(jnp.array([[0.0, 1.0, 0.0, 0.0]]),(g_batch.shape[0],1)),
                                                jnp.tile(jnp.array([[1.0, 0.0, 0.0, 0.0]]),(g_batch.shape[0],1)),model_fns2)
    out_y, out_yt = equ_func2(all_params2, g_batch, jnp.tile(jnp.array([[0.0, 0.0, 1.0, 0.0]]),(g_batch.shape[0],1)),
                                                jnp.tile(jnp.array([[1.0, 0.0, 0.0, 0.0]]),(g_batch.shape[0],1)),model_fns2)
    out_z, out_zt = equ_func2(all_params2, g_batch, jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(g_batch.shape[0],1)),
                                                jnp.tile(jnp.array([[1.0, 0.0, 0.0, 0.0]]),(g_batch.shape[0],1)),model_fns2)
    out, out_t = equ_func(all_params2, g_batch, jnp.tile(jnp.array([[1.0, 0.0, 0.0, 0.0]]),(g_batch.shape[0],1)),model_fns2)
    
    out2, out2_t = equ_func(all_params, g_batch, jnp.tile(jnp.array([[1.0, 0.0, 0.0, 0.0]]),(g_batch.shape[0],1)),model_fns)
    out2_x, out2_xx = equ_func2(all_params, g_batch, jnp.tile(jnp.array([[0.0, 1.0, 0.0, 0.0]]),(g_batch.shape[0],1)),
                                                jnp.tile(jnp.array([[0.0, 1.0, 0.0, 0.0]]),(g_batch.shape[0],1)),model_fns)
    out2_y, out2_yy = equ_func2(all_params, g_batch, jnp.tile(jnp.array([[0.0, 0.0, 1.0, 0.0]]),(g_batch.shape[0],1)),
                                                jnp.tile(jnp.array([[0.0, 0.0, 1.0, 0.0]]),(g_batch.shape[0],1)),model_fns)
    out2_z, out2_zz = equ_func2(all_params, g_batch, jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(g_batch.shape[0],1)),
    ...
    pT = jnp.concatenate([out2[:,(k+3):(k+4)]*all_params["data"][keys2[k]] for k in range(len(keys2))],1)
    uvwp = jnp.concatenate([uvwp, pT],1)
     
    uts2 =  jnp.concatenate([out2_t[:,(k+3):(k+4)]*all_params["data"][keys2[k]]/all_params["domain"]["domain_range"]['t'][1] for k in range(len(keys2))],1)
    uxs2 =  jnp.concatenate([out2_x[:,(k+3):(k+4)]*all_params["data"][keys2[k]]/all_params["domain"]["domain_range"]['x'][1] for k in range(len(keys2))],1)
    uys2 =  jnp.concatenate([out2_y[:,(k+3):(k+4)]*all_params["data"][keys2[k]]/all_params["domain"]["domain_range"]['y'][1] for k in range(len(keys2))],1)
    uzs2 =  jnp.concatenate([out2_z[:,(k+3):(k+4)]*all_params["data"][keys2[k]]/all_params["domain"]["domain_range"]['z'][1] for k in range(len(keys2))],1)
    uxxs2 = jnp.concatenate([out2_xx[:,(k+3):(k+4)]*all_params["data"][keys2[k]]/all_params["domain"]["domain_range"]['x'][1]/all_params["domain"]["domain_range"]['x'][1] for k in range(len(keys2))],1)
    uyys2 = jnp.concatenate([out2_yy[:,(k+3):(k+4)]*all_params["data"][keys2[k]]/all_params["domain"]["domain_range"]['y'][1]/all_params["domain"]["domain_range"]['y'][1] for k in range(len(keys2))],1)
    uzzs2 = jnp.concatenate([out2_zz[:,(k+3):(k+4)]*all_params["data"][keys2[k]]/all_params["domain"]["domain_range"]['z'][1]/all_params["domain"]["domain_range"]['z'][1] for k in range(len(keys2))],1)
    
    grad_p = jnp.concatenate([uxs2[:,0:1], uys2[:,0:1], uzs2[:,0:1]],1)
    grad_T = jnp.concatenate([uts2[:,1:2], uxs2[:,1:2], uys2[:,1:2], uzs2[:,1:2], uxxs2[:,1:2], uyys2[:,1:2], uzzs2[:,1:2]],1)
    ...
    return uvwp, uxs, uys, uzs, Tx, Ty, Q, uts, uxxs, uyys, uzzs, grad_p, grad_T
...
def Tecplotfile_gen(c, path, name, all_params, all_params2, all_params3, domain_range, output_shape, order, timestep, is_ground, is_mean, model_fn, model_fn2=None):
    ...
    uvwp, uxs, uys, uzs, Tx, Ty, Q, uts, uxxs, uyys, uzzs, grad_p, grad_T = zip(*[Derivatives(dynamic_params, dynamic_params2, all_params, all_params2, eval_grid[i:i+10000], model_fn, model_fn2)
    ...
    uts = np.concatenate(uts, axis=0)
    uxxs = np.concatenate(uxxs, axis=0)
    uyys = np.concatenate(uyys, axis=0)
    uzzs = np.concatenate(uzzs, axis=0)
    grad_p = np.concatenate(grad_p, axis=0)
    grad_T = np.concatenate(grad_T, axis=0)
    ...
    np.save(path + 'npyresult/' + name + f'/ts_{timestep:02d}' + '.npy', np.concatenate([eval_grid_e, uvwp, uxs, uys, uzs, Tx, Ty, uts, uxxs, uyys, uzzs, grad_p, grad_T], axis=1))
#%%
if __name__ == "__main__":
    ...
    print(values)
    with open(os.path.dirname(cur_dir)+ '/' + data['path'] + values[4]['model_name'] +'/summary/constants.pickle','rb') as f:
        constants2 = pickle.load(f)
    values2 = list(constants2.values())
    ...
    c2 = Constants(run = values2[0],
                domain_init_kwargs = values2[1],
                data_init_kwargs = values2[2],
                network1_init_kwargs = values2[3],
                problem_init_kwargs = values2[5],
                optimization_init_kwargs = values2[6],
                equation_init_kwargs = values2[7],)
    ...
    checkpoint_list2 = sorted(glob(run2.c.model_out_dir+'/*.pkl'), key=lambda x: int(x.split('_')[-1].split('.')[0]))
    print(run2.c.model_out_dir)
    ...
    all_params2, model_fn2, train_data2 = run2.test()
    model = Model(all_params["network1"]["layers"], model_fn)
    model2 = Model(all_params2["network1"]["layers"], model_fn2)
    
    all_params["network1"]["layers"] = from_state_dict(model, model_params).params
    all_params2["network1"]["layers"] = from_state_dict(model2, model_params2).params
    all_params3 = all_params.copy()
    ...
    try:
        ...
        all_params3['domain']['grid_size'] = output_shape
        all_params3['domain']['domain_range'] = domain_range
        all_params3['domain']['bound_keys'] = bound_keys
    ...
    for timestep in timesteps:
        Tecplotfile_gen(c, path, args.foldername, all_params, all_params2, all_params3, domain_range, output_shape, order, timestep, is_ground, is_mean, model_fn, model_fn2)
